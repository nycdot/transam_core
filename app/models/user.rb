...
class User < ActiveRecord::Base
    ...
    def assigned_tasks
        Task.where('assigned_to_user_id = ? AND state IN (?)', id, %w(new started halted)).order(:complete_by)
    end
    ...
    def is_in_roles?(roles_to_test)
        roles_to_test.each do |name|
            return true if roles_name.include? name.to_s
        end
        false
    end
    ...
    def update_user_organization_filters
        UserOrganizationFilter.where('resource_type IS NOT NULL').each do |filter|
            puts try(filter.resource_type.downcase.pluralize).include? filter.resource
            puts organizations.inspect
            if respond_to? filter.resource_type.downcase.pluralize # check has many associations
                if try(filter.resource_type.downcase.pluralize).include? filter.resource
                    user_organization_filters << filter
                end
            elsif respond_to? filter.resource_type.downcase # check single association
                if try(filter.resource_type.downcase) == filter.resource
                    user_organization_filters << filter
                end
            end
        end

        if user_organization_filters.system_filters.count == 0
            f = UserOrganizationFilter.new(active: 1, name: "#{name}'s organizations", description: "#{name}'s organizations", sort_order: 1)
            f.users = [self]
            f.creator = User.find_by(first_name: 'system')
            f.query_string = Organization.active.joins(:users).where('users_organizations.user_id = ?', id).to_sql
            f.save!
        end

        if user_organization_filter.nil? || !(user_organization_filters.include? user_organization_filter)
            self.user_organization_filter = user_organization_filters.system_filters.first
        end
